...
const validateInput = (input) => {
  ...
  access(filePath, (constants || fs).F_OK | (constants || fs).R_OK, (err) => {
    if (err) {
...
const validateOutput = (output) => {
  ...
  access(filePath, (constants || fs).F_OK | (constants || fs).W_OK, (err) => {
    if (err) {
      process.stderr.write(
        chalk.bold.red(
          `${output} ${
...
const validateArgs = (options) => {
  ...
  if (!shift || !action) {
    errors.push(
      `${
        !shift && !action
          ? "A shift and an action (encode/decode) are"
          : action
          ? "A shift is"
          : "An action (encode/decode) is"
      } required.\n`
    );
  } else {
